...
assert.strictEqual(countAll().categories, 4);
assert.ok(countAll().products >= 4, "expected seeded products present (count drifts via admin)");
cats.forEach((c) => {
  assert.ok(
...
// --- category image edit ---
const pizza = getCategoryBySlug("pizza");
assert.ok(pizza, "pizza category must exist");
const originalPizzaImage = pizza.imageUrl;
updateCategoryImage(pizza.id, "https://images.unsplash.com/photo-1574071318508-1cdbab80d002");
assert.strictEqual(
  getCategoryBySlug("pizza")!.imageUrl,
  "https://images.unsplash.com/photo-1574071318508-1cdbab80d002",
  "category image must be updated",
);
updateCategoryImage(pizza.id, originalPizzaImage);
console.log("✓ category image update verified");

// --- product CRUD ---
const before = listProductsByCategory(pizza.id);
createProduct({
  categoryId: pizza.id,
...
const created = listProductsByCategory(pizza.id).find((p) => p.name === "طبق اختبار تلقائي");
assert.ok(created, "created product must be readable");

// --- ingredient CRUD ---
const cheeseId = createIngredient(created.id, "جبنة إضافية", 500, 1, 0);
const onionId = createIngredient(created.id, "بصل", 0, 0, 1);
const ings = listIngredientsByProduct(created.id);
...
updateIngredient(onionId, "بصل مكرمل", 50, 0, 0);
const onionAfter = listIngredientsByProduct(created.id).find((i) => i.id === onionId);
...
deleteIngredient(onionId);
assert.strictEqual(
  listIngredientsByProduct(created.id).length,
  1,
...
const updated = getProductById(created.id);
...
assert.strictEqual(
  listProductsByCategory(pizza.id).length,
  before.length,
...
const orderId = createOrder(JSON.stringify(["بيتزا × 2 — 56.00 دج"]), 6600);
...
deleteOrder(orderId);
assert.strictEqual(
  listOrders().find((o) => o.id === orderId),
  undefined,
